...
def _margin_book():
    ...
    cj = importlib.util.spec_from_file_location('_mb', os.path.join(_HERE, 'margin_book.py'))
    m = importlib.util.module_from_spec(cj)
    cj.loader.exec_module(m)
    return m

def _scanner():
    """Reuse basis_scanner's fetchers so plan and scan cannot drift apart."""
    cj = importlib.util.spec_from_file_location('_bs', os.path.join(_HERE, 'basis_scanner.py'))
    m = importlib.util.module_from_spec(cj)
    cj.loader.exec_module(m)
    return m
...
class Config(BaseModel):
    ...
    size_usd: float = Field(default=45.0, description='Baseline notional per entry, in QUOTE currency (USD). Emitted as a ready total_amount in base units, already rounded to whole min_notional slices.')
    size_scaled_usd: float = Field(default=90.0, description='The scaled-up notional, used when a pair has already realized well.')
...
def _mark_from(bz: dict) -> float | None:
    try:
        ...
        cq = float(bz.get('unrealized_pnl') or 0)
        return ay + cq / ad if ad and ay > 0 else None
    except Exception:
        return None
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    import aiohttp
    az = []
    async with aiohttp.ClientSession(timeout=an.TIMEOUT) as cd:
        (bk, bl), (ak, al) = await asyncio.gather(an._hl_all(cd), an._bin_all(cd))
    az += [e for e in (bl, al) if e]
    try:
        ck = await client.portfolio.get_state()
    except Exception as e:
        ck = {}
        az.append(f'portfolio.get_state: {e}')
    try:
        cc = await client.trading.get_positions()
        ca = cc.get('data', cc) if isinstance(cc, dict) else cc or []
    except Exception as e:
    ...
    cu = [config.maker_venue_hl, config.taker_venue_bin]
    bt = _margin_book().compute_venue_margin(ck, ca, cu, config.account_name, config.dust_usd)
    ...
    for ag, h in bj.items():
        cn = ag in bk and an._bin_symbol(ag) in ak
        if not cn:
            be.append((ag, h))
        elif len(h['legs']) < 2:
            co.append((ag, h))
    ...
    for ag, h in bx.items():
        ...
        cg = h['legs'].get(config.maker_venue_hl)
        cf = h['legs'].get(config.taker_venue_bin)
        if not bo or not aj:
            cp.append({'base': ag, 'urgent': False, 'fundsig': None, 'carry': None, 'exit_bps': ah, 'why': 'no funding data', 'notional': h['notional'], 'upnl': h['upnl'], 'legs': h['legs']})
            continue
        if cg and cf and (cg != cf):
            ce = cg == 'SHORT'
        elif cg:
            ce = cg == 'SHORT'
        else:
        ...
        bg = bd - bc if ce else bc - bd
        am = float(aj.get('interval_h') or 8.0)
        bp = max(1.0, am)
        import time as _t
        ao, _ = an._leg_carry_bps(bo['rate_per_print'], 1.0, 3600.0 - _t.time() % 3600.0, bp, ce, 1.0)
        ap, _ = an._leg_carry_bps(aj['rate_per_print'], am, max(0.0, (aj.get('next_print_min') or 0) * 60.0), bp, not ce, 1.0)
        ar = ao + ap
        bb = ah + ar
        cr = bg < config.urgent_fundsig_bph
        cp.append({'base': ag, 'urgent': cr, 'fundsig': bg, 'carry': ar, 'exit_bps': bb, 'notional': h['notional'], 'upnl': h['upnl'], 'legs': h['legs'], 'short_hl': ce, 'why': 'funding turned against us' if cr else 'carrying fine'})
    aq = config.max_notional_per_line
    if aq:
        cp = [u for u in cp if u['notional'] <= aq]
    cr = sorted([u for u in cp if u['urgent']], key=lambda u: u['fundsig'] or 0)
    bv = sorted([u for u in cp if not u['urgent']], key=lambda u: u['carry'] or 0)
    ...
    for ag in [b for b in bk if an._bin_symbol(b) in ak and an._symbol_ok(b)[0]]:
        ...
        if bg >= at.min_fundsig_bph and ar > 0 and (aw >= at.min_edge_bps):
            ...
            cl = config.taker_venue_bin if v == 'HL' else config.maker_venue_hl
            cb = (bo if v == 'HL' else aj)['mark'] or 0.0

            def _amt(cs):
                ch = max(1, int(cs // config.min_notional))
                return ch * config.min_notional / cb if cb else 0.0
            ax.append({'px': cb, 'amount_base': _amt(config.size_usd), 'amount_scaled': _amt(config.size_scaled_usd), 'base': ag, 'dir': f'{v}.{s}', 'edge': aw, 'carry': ar, 'fundsig': bg, 'held': ag in bj, 'maker_connector': br, 'maker_trading_pair': _pair_for(ag, br), 'taker_connector': cl, 'taker_trading_pair': _pair_for(ag, cl), 'maker_side_str': s})
    ...
    for u in bv:
        ...
        ct += 1
    L = [f'SLOT PLAN   {ct}/{config.slots} slots allocated']
    if ab:
    ...
    if co:
        cm = sum((h['notional'] for _, h in co))
        L += ['', f'  !! UNHEDGED: {len(co)} position(s) on tradeable bases, ${cm:,.0f} notional - {', '.join((b for b, _ in co[:8]))}' + (' ...' if len(co) > 8 else ''), '  One leg only on a base this strategy CAN trade. In a DEDICATED account (the race)', '  that means one thing: our own failed hedge - naked directional risk on a book that', '  should be flat. Re-hedge or close it before opening anything new; there is no', '  operator during the race.', f'  CAVEAT on a shared account: only {', '.join((v.split('_')[0] for v in cu))} are', '  read here, so a leg hedged on any OTHER venue is invisible and shows up in this', '  list. Confirm against the full venue set before acting on a dev box.']
    if be:
        cm = sum((h['notional'] for _, h in be))
        L += ['', f'  FOREIGN: {len(be)} position(s), ${cm:,.0f} notional - {', '.join((b for b, _ in be[:8]))}' + (' ...' if len(be) > 8 else ''), '  On bases NOT listed on both venues, so this strategy could never have opened', '  them. EXCLUDED from the plan and never traded.']
    if az:
        L += ['', '!! ' + ' | '.join(az)]
    if ai is not None and ai < config.margin_floor_pct:
    ...
    ci = config.slots - ct
    L.append('')
    L.append(f'  {len(cr)} urgent unwind(s), {len(ax)} entry candidate(s), {len(bv)} normal unwind(s) available. {ci} slot(s) idle.')
    if ci and bv:
