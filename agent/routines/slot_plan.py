...
class Config(BaseModel):
    ...
    margin_floor_pct: float = Field(default=0.15, description='Below this margin health on the BINDING venue, plan no new entries - unwinds only.')
    ...
    max_base_notional_quote: float = Field(default=0.0, description='Cap on TOTAL notional per base across both legs (0 = uncapped). Counts what is already held, so repeated ADDs cannot compound past it.')
